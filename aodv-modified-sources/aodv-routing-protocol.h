 ...
 */
 ...
 public:
   ...
   void SetIsBlackhole(bool status) { m_isBlackhole = status; }
   void SetIsBlackholeScenario3(bool status) { m_isBlackhole_Scenario_3 = status; }

   /**
   * \brief Starts the RREQ flooding attack.
   */
   void StartRreqFlood();

   /**
   * \brief Stops the RREQ flooding attack.
   */
   void StopRreqFlood();
 ...
 private:
   ...
   bool m_isBlackhole = false; // Δική μου προσθήκη
   bool m_isBlackhole_Scenario_3 = false; // Δική μου προσθήκη
   //\}
 ...
 private:
   ...
   bool m_isFlooding = false;
   
   /// Timer for the RREQ flood attack
   Timer m_floodTimer;
   
   /**
   * \brief Periodically sends a bogus RREQ packet.
   */
   void SendBogusRreq();
