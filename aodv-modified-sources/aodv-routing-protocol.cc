...
{
    // Δική μου προσθήκη
    // Ορισμός των μεταβλητών στην αρχή της συνάρτησης
    Ipv4Address dst = header.GetDestination ();
    Ipv4Address origin = header.GetSource ();

    // Λογική Επίθεσης
    /*
    if (m_isBlackhole_Scenario_3)
    {
        NS_LOG_WARN ("[ATTACK] Aggressive Blackhole Node " << m_ipv4->GetObject<Node>()->GetId()
                     << " is dropping packet and sending fake RERR to " << origin);
    ...
    */
    if (m_isBlackhole)
    {
        if (m_isBlackhole_Scenario_3)
        {
            NS_LOG_WARN ("[ATTACK] Blackhole Node " << m_ipv4->GetObject<Node>()->GetId()
                        << " is dropping packet and sending fake RERR to " << origin);
            
            SendRerrWhenNoRouteToForward(dst, 0, origin); // Αποστολή RERR
        }
        else
        {
            NS_LOG_WARN ("[ATTACK] Blackhole Node " << m_ipv4->GetObject<Node>()->GetId()
                        << " is FORWARD-dropping data packet UID " << p->GetUid ());
        }

        return false; // Σιωπηλή απόρριψη πακέτων
    }

    // Τέλος λογικής επίθεσης

    // Τέλος δικής μου προσθήκης
...
{
 NS_LOG_WARN ("--- RecvRequest CALLED on Node " << m_ipv4->GetObject<Node>()->GetId() << " at " << Simulator::Now ().GetSeconds () << "s ---");
 NS_LOG_FUNCTION (this);
 RreqHeader rreqHeader;
 ...
 if (m_isBlackhole)
 {
     // Αν είμαι ο κακόβουλος κόμβος:
     // 1. ΜΗΝ προωθήσεις το RREQ
     // 2. Στείλε ΑΜΕΣΩΣ ένα fake RREP πίσω στον αποστολέα του RREQ (τον 'src'), ισχυριζόμενος ότι έχεις μια φανταστική διαδρομή προς τον τελικό προορισμό

     NS_LOG_WARN("[ATTACK] Blackhole Node " << m_ipv4->GetObject<Node>()->GetId()
                 << " is hijacking RREQ for destination " << rreqHeader.GetDst());

     // Δημιουργία ενός fake RREP
     RrepHeader fakeRrep;

     fakeRrep.SetDst(rreqHeader.GetDst()); // Ο προορισμός είναι αυτός που ζητήθηκε
     //fakeRrep.SetDstSeqno(999999); // Ένας τεράστιος αριθμός ακολουθίας για να φανεί η διαδρομή πρόσφατη
     fakeRrep.SetOrigin(rreqHeader.GetOrigin()); // Ο origin είναι αυτός που έκανε την αρχική αναζήτηση
     fakeRrep.SetLifeTime(m_myRouteTimeout * 10); // Μια πολύ μεγάλη διάρκεια ζωής για τη διαδρομή
     fakeRrep.SetHopCount(1); // Ισχυρισμός ότι είμαστε μόνο ένα hop μακριά

     // Αν το RREQ περιέχει έναν γνωστό αριθμό ακολουθίας, τον αυξάνουμε κατά ένα.
     // Αυτό κάνει τη διαδρομή μας να φαίνεται εγγυημένα πιο "φρέσκια".
     // Αλλιώς, χρησιμοποιούμε έναν τεράστιο αριθμό για να καθιερώσουμε τη διαδρομή μας ως την καλύτερη.
     if (!rreqHeader.GetUnknownSeqno())
     {
        fakeRrep.SetDstSeqno(rreqHeader.GetDstSeqno() + 1);
        NS_LOG_WARN("[ATTACK] Replying with targeted SeqNo: " << rreqHeader.GetDstSeqno() + 1);
     }
     else
     {
        // Χρησιμοποιούμε έναν πολύ μεγάλο αριθμό, αλλά που σχετίζεται με τον τρέχοντα seqNo του κόμβου
        // για να αποφύγουμε σταθερές τιμές που μπορεί να είναι ανιχνεύσιμες.
        fakeRrep.SetDstSeqno(m_seqNo + 1000); 
        NS_LOG_WARN("[ATTACK] Replying with large fabricated SeqNo: " << m_seqNo + 1000);
     }
     ...
     // Εύρεση του socket για την αποστολή της απάντησης πίσω στον αποστολέα του RREQ (src)
     RoutingTableEntry toPrevHop;
     if (m_routingTable.LookupRoute(src, toPrevHop))
     {
         Ptr<Socket> socket = FindSocketWithInterfaceAddress(toPrevHop.GetInterface());
         if (socket)
         {
             socket->SendTo(packet, 0, InetSocketAddress(src, AODV_PORT));
             NS_LOG_WARN("[ATTACK] Sent aggressive fake RREP to " << src);
         }
     }

     // Παύση της επεξεργασίας του κανονικού RREQ. Ο κακόβουλος κόμβος δεν το προωθεί ποτέ.
     return;
 }
 // Τέλος προσθήκης
...
// Δική μου προσθήκη (μέχρι το τέλος)

void
RoutingProtocol::StartRreqFlood()
{
  if (m_isBlackhole)
  {
    NS_LOG_WARN("[ATTACK] Starting RREQ Flood attack on Node " << m_ipv4->GetObject<Node>()->GetId());
    m_isFlooding = true;
    m_floodTimer.SetFunction(&RoutingProtocol::SendBogusRreq, this);
    SendBogusRreq(); // Άμεση έναρξη
  }
}

void
RoutingProtocol::StopRreqFlood()
{
  NS_LOG_WARN("[ATTACK] Stopping RREQ Flood attack on Node " << m_ipv4->GetObject<Node>()->GetId());
  m_isFlooding = false;
  m_floodTimer.Cancel();
}

void
RoutingProtocol::SendBogusRreq()
{
  if (!m_isFlooding)
  {
    return;
  ...
  NS_LOG_INFO("[ATTACK] Sending bogus RREQ flood packet.");

  // Δημιουργία RREQ για έναν ανύπαρκτο προορισμό
  RreqHeader rreqHeader;
  rreqHeader.SetDst(Ipv4Address("99.99.99.99")); // Ψεύτικη IP
  rreqHeader.SetUnknownSeqno(true);
  ...
  // Αποστολή από όλες τις διεπαφές
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
  {
    Ptr<Socket> socket = j->first;
  ...
  // Προγραμματισμός της επόμενης αποστολής για να δημιουργηθεί η πλημμύρα
  m_floodTimer.Schedule(Seconds(0.1)); // Αποστολή 10 ψεύτικων RREQ ανά δευτερόλεπτο
}
